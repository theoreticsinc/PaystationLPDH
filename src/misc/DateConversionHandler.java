...
public class DateConversionHandler {
    ...
    public static void main(String[] av) {
        ...
        //log.info(dch.convertDate2DBbase("07/19/17 00:00:0.00"));
        log.info(dch.convertDate2DBbase("07/19/17"));
        
        log.info("NOW:" + dch.convertNow2Sec());
        //log.info("NOW in Milli:" + dch.convertNow2MilliSec());
        dch.convertDateLong2Sec( String.valueOf(dch.convertNow2Sec()));
        long secs = 0;
        try {
            log.info("2017-05-27 23:37:50.0:" + dch.convertDate2Sec("2017-05-27 23:37:50.0"));
            ...
            log.info("D0 converted unixDate:" + secs);
            
            Date d1 = dch.convertJavaUnixTime2Date(1500007020);
            
            log.info("D1 converted unixDate:" + d1.toString());
            Date d2 = dch.convertJavaUnixTime2Date(1500018415); 
            
            log.info("D2 converted unixDate:" + d2.toString());
            
            Date d3 = dch.convertJavaUnixTime2Date(1500040800);
            
            log.info("D3 converted unixDate:" + d3.toString());

            
        } catch (ParseException ex) {
            log.error(ex.getMessage());
        }
        log.info(dch.getHoursfromseconds(3600));
        log.info(dch.getRemainingMinutesfromseconds(60));
    }
    ...
    public Date getElapsedTimeFromUnixTime(String timestampStr) {
        ...
        log.info("getElapsedTimeFromUnixTime:" + d2.toString());
        return retDate;
    }
    ...
    public long convertArduinoDate2UnixTime(Date date) {
        ...
        // Minus 8 from Arduino
            ...
            log.info("Unix Time:" + unixTime);//<- prints 1352504418   
        
        return unixTime;
    }
    
    public long convertEverArduinoDate2UnixTime(Date date) {
        long unixTime = 0;
        // Minus 8 from Arduino
            date.setHours(date.getHours() + 8);
            date.setMinutes(date.getMinutes());
...
//            log.info("unixDate:" + d.toString());
    ...
    public long convertDateLong2Sec(String datetext) {// the datetext is already in date.long format - - -
        ...
        log.info("--ParkerStamp--");
        log.info(df.format(ParkerStamp));
        log.info(tf.format(ParkerStamp));
        long noOFsecs = 0;
    ...
    public long GetYearinSecfrom2000(int year) {
        ...
        while (loopbackto2000 != 0) {
            cumulativeyears = cumulativeyears + this.convertyears2secs(loopbackto2000);//31622400;
            loopbackto2000--;
        }
