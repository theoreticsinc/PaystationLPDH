 ...
 */
public class SerialEpsonHandler {

    static Logger log = LogManager.getLogger(SerialEpsonHandler.class.getName());
    ...
    public static void main(String[] args) {
        try {
            SerialEpsonHandler ea = new SerialEpsonHandler();
            ea.closeprinter();
            ea.openPrinter();
...
//            ea.selectstyle((short) 0);
    ...
    public void closePrinter() {
        ...
        /*
        if (null != PrnPortType && PrnPortType.startsWith("COM")) {
            if (null != serialPort)
                serialPort.close();
        } else if (null != PrnPortType && PrnPortType.startsWith("LPT")) {
    ...
    public void openPrinter() {
        try {
            ...
            if (PrinterEnabled) {
                ...
                CommPortOwnershipListener cL = new CommPortOwnershipListener() {
                    @Override
                ...
                portId.addPortOwnershipListener(cL);
                if (portId.isCurrentlyOwned()) {
                    log.info("Error: Port is currently in use");
                    do {
                        log.info("Waiting till port is timed out");
                    } while (portId.isCurrentlyOwned());
                    serialPort = (SerialPort) portId.open("Next Printing", 10);
                    serialPort.setSerialPortParams(9600, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
                } else {
                    if (null != serialPort) {
                        serialPort = (SerialPort) portId.open("Java Printing", 10);
                        serialPort.setSerialPortParams(9600, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
                    } else {
                        serialPort = (SerialPort) portId.open("Java Printing", 10);
                        serialPort.setSerialPortParams(9600, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
